...
function firstscript() {
	...
	document.images.image.src = "Data_jin/Data_jin_jpg/data0/image010.jpg";
	filename = "Data_jin/Data_jin_jpg/data";
	image_type = ".jpg"
	image_load(img_op, filename);
	}
}
...
function kakudai_Click(kakudai, ischecked){
	...
	if(ischecked == true){
		// 拡大画像表示       (框被选中)
		if(isSupportWebp)	filename = "Data_jin/Data_jin_k-6_Webp/data";
		else	filename = "Data_jin/Data_jin_k-6_jpg/data";
		image_loadk_6(img_k_6, filename);
		
		count_y = 4;
		count_x = 10;
		image.src = img_k_6[4][10].src;
		image_ar = img_k_6;
		kakudai_Flag = 1;
	}
	else{
		// デフォルト画像表示      (框未被选中)
		image.src = img_op[0][10].src;
		image_ar = img_op;
		count_y = 0;
		count_x = 10;
		kakudai_Flag = 0;
...
function kakudai_bar(value){
	if(value == 0){
		...
		if(isSupportWebp)	filename = "Data_jin/Data_jin_k-5_Webp/data";
		else	filename = "Data_jin/Data_jin_k-5_jpg/data";
		image_loadk_5(img_k_5, filename);
		image.src = img_k_5[count_y][count_x].src;
		image_ar = img_k_5;
	}
	else if(value == 2){
		if(isSupportWebp)	filename = "Data_jin/Data_jin_k-4_Webp/data";
		else	filename = "Data_jin/Data_jin_k-4_jpg/data";
